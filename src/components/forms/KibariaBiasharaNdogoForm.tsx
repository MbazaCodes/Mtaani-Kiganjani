...
export const KibariaBiasharaNdogoForm: React.FC<FormProps> = ({
  onSubmit,
  isLoading,
  lang = "sw",
  userProfile,
  ...
  const [vals, setVals] = useState<FormVals>({
    ...
    owner_dob: userProfile?.date_of_birth || "",
    owner_sex: userProfile?.sex || "",
    signature: "",
  });
  ...
  const validate = (): boolean => {
    ...
    if (step === "biashara") {
      if (!vals.business_name.trim())
        e.business_name = L(lang, "Jina la biashara linahitajika", "Business name required");
      if (!vals.business_type)
        e.business_type = L(lang, "Aina ya biashara inahitajika", "Business type required");
      if (!vals.location_address.trim())
        e.location_address = L(lang, "Anwani ya biashara inahitajika", "Business address required");
    }
    if (step === "mmiliki") {
      if (!vals.owner_name.trim())
        e.owner_name = L(lang, "Jina la mmiliki linahitajika", "Owner name required");
      if (!vals.owner_nida.trim())
        e.owner_nida = L(lang, "Namba ya NIDA inahitajika", "NIDA number required");
      if (!vals.owner_phone.trim())
        e.owner_phone = L(lang, "Namba ya simu inahitajika", "Phone number required");
    }
    setErrors(e);
  ...
  const inputCls = (k: string) =>
    `w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-colors ${errors[k] ? "border-red-400 bg-red-50" : "border-stone-200 bg-white"}`;
    ...
    return (
      <div className="text-center py-10 space-y-4">
        ...
        <p className="text-stone-500 text-sm">
          {L(
            lang,
            "Kibari cha biashara kitashughulikiwa na ofisi ya mtaa.",
            "Your business permit will be processed by the ward office.",
          )}
        </p>
        <div className="bg-stone-50 rounded-xl p-4 text-left max-w-sm mx-auto space-y-2">
          {[
            [L(lang, "Biashara", "Business"), vals.business_name],
            [
              L(lang, "Aina", "Type"),
              BUSINESS_TYPES.find((t) => t.value === vals.business_type)?.label || "",
            ],
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2 justify-center">
        {STEPS.map((s, i) => (
          <div
            key={s.key}
            className={`flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold ${i <= stepIdx ? "bg-emerald-100 text-emerald-700" : "bg-stone-100 text-stone-400"}`}
          >
        ...
        <div className="space-y-4">
          <div className={secHdr}>
            <p className="font-bold text-emerald-800 text-sm flex items-center gap-2">
              <Store size={16} />
              {L(lang, "TAARIFA ZA BIASHARA", "BUSINESS INFORMATION")}
            </p>
          </div>
          <div>
            <label className={lbl}>{L(lang, "Jina la Biashara *", "Business Name *")}</label>
            <input
              value={vals.business_name}
              onChange={(e) => {
                set("business_name", e.target.value);
                clrErr("business_name");
              }}
              className={inputCls("business_name")}
              placeholder={L(lang, "Mfano: Duka la Amina", "E.g. Amina's Shop")}
            />
          </div>
          ...
          <div>
            <label className={lbl}>{L(lang, "Aina ya Biashara *", "Business Type *")}</label>
            <select
              value={vals.business_type}
              onChange={(e) => {
                set("business_type", e.target.value);
                clrErr("business_type");
              }}
              className={inputCls("business_type")}
            >
          ...
          {vals.business_type === "OTHER" && (
            <div>
              <label className={lbl}>
                {L(lang, "Eleza Aina ya Biashara *", "Describe Business Type *")}
              </label>
              <input
                value={vals.business_type_other}
                onChange={(e) => set("business_type_other", e.target.value)}
                className={inputCls("business_type_other")}
              />
            </div>
          )}
          <div>
            <label className={lbl}>
              {L(lang, "Shughuli Kuu ya Biashara", "Main Business Activity")}
            </label>
            <textarea
              value={vals.business_activity}
              onChange={(e) => set("business_activity", e.target.value)}
              rows={2}
              className={`${inputCls("business_activity")} resize-none`}
              placeholder={L(
                lang,
                "Eleza biashara yako kwa ufupi...",
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className={lbl}>
                {L(lang, "Idadi ya Wafanyakazi", "Number of Employees")}
              </label>
              <input
                type="number"
                min="1"
                value={vals.employees_count}
                onChange={(e) => set("employees_count", e.target.value)}
                className={inputCls("employees_count")}
              />
            </div>
            <div>
              <label className={lbl}>{L(lang, "Tarehe ya Kuanza", "Start Date")}</label>
              <input
                type="date"
      ...
      {step === "mmiliki" && (
        <div className="space-y-4">
          <div className={secHdr}>
            <p className="font-bold text-emerald-800 text-sm flex items-center gap-2">
              <User size={16} />
              {L(lang, "TAARIFA ZA MMILIKI", "OWNER INFORMATION")}
            </p>
          </div>
          <div>
            <label className={lbl}>
              {L(lang, "Jina Kamili la Mmiliki *", "Full Owner Name *")}
            </label>
            <input
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  set("owner_phone", e.target.value);
                  clrErr("owner_phone");
                }}
            ...
            <div>
              ...
              <select
                value={vals.owner_sex}
                onChange={(e) => set("owner_sex", e.target.value)}
                className={inputCls("owner_sex")}
              >
      ...
      {step === "preview" && (
        <div className="space-y-4">
          <div className={secHdr}>
            <p className="font-bold text-emerald-800 text-sm flex items-center gap-2">
              <Eye size={16} />
              {L(lang, "HAKIKI TAARIFA", "PREVIEW INFORMATION")}
            </p>
          </div>
          <div className="bg-white border border-stone-200 rounded-xl divide-y divide-stone-100">
            {[
              [L(lang, "Jina la Biashara", "Business Name"), vals.business_name],
              [
                L(lang, "Aina", "Type"),
                BUSINESS_TYPES.find((t) => t.value === vals.business_type)?.label ||
                  vals.business_type_other,
              ],
              [L(lang, "Anwani", "Address"), vals.location_address],
              [L(lang, "Mmiliki", "Owner"), vals.owner_name],
              [L(lang, "NIDA", "NIDA"), vals.owner_nida],
              [L(lang, "Simu", "Phone"), vals.owner_phone],
              [L(lang, "Ada", "Fee"), `TSh ${SERVICE_FEE.toLocaleString()}`],
            ].map(([k, v]) => (
              <div key={k} className="flex justify-between px-4 py-2.5 text-sm">
                <span className="text-stone-500">{k}</span>
                <span className="font-bold text-stone-800 text-right max-w-[60%]">{v}</span>
              </div>
            ))}
          </div>
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex gap-2">
            ...
            <p className="text-xs text-amber-700">
              {L(
                lang,
                "Taarifa zote ni sahihi. Utumie namba ya ombi kupata kibari baada ya idhini.",
                "All information is accurate. Use your reference number to collect the permit after approval.",
              )}
            </p>
          </div>
        </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        {step !== "preview" ? (
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl transition-colors disabled:opacity-50"
          >
