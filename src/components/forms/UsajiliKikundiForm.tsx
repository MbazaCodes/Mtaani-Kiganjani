...
const emptyLeader = (role: string): Leader => ({
  first_name: "",
  last_name: "",
  nida: "",
  ct_id: "",
...
export const UsajiliKikundiForm: React.FC<FormProps> = ({
  onSubmit,
  isLoading,
  lang = "sw",
  userProfile,
  ...
  const removeMember = (i: number) =>
    setVals((p) => ({ ...p, members: p.members.filter((_, idx) => idx !== i) }));
  ...
  const validate = (): boolean => {
    ...
    if (step === "kikundi") {
      if (!vals.group_name.trim())
        e.group_name = L(lang, "Jina la kikundi linahitajika", "Group name required");
      if (!vals.group_type)
        e.group_type = L(lang, "Aina ya kikundi inahitajika", "Group type required");
      if (!vals.group_purpose.trim())
        e.group_purpose = L(lang, "Madhumuni yanahitajika", "Purpose required");
    }
    if (step === "mahali") {
    ...
    if (step === "viongozi") {
      vals.leaders.forEach((l, i) => {
        if (!l.first_name.trim())
          e[`lfn_${i}`] = L(lang, "Jina la kwanza linahitajika", "First name required");
        if (!l.last_name.trim())
          e[`lln_${i}`] = L(lang, "Jina la mwisho linahitajika", "Last name required");
        if (!l.nida.trim()) e[`lnida_${i}`] = L(lang, "NIDA inahitajika", "NIDA required");
        if (!l.ct_id.trim())
          e[`lct_${i}`] = L(
            lang,
            "CT ID inahitajika — kiongozi lazima awe mwanachama wa E-Mtaa",
            "CT ID required — leader must be registered on E-Mtaa",
          );
    ...
    return (
      <div className="text-center py-10 space-y-4">
        ...
        <h3 className="text-xl font-black">
          {L(lang, "Usajili Umefanikiwa!", "Registration Submitted!")}
        </h3>
        <p className="text-stone-500 text-sm">
          {L(
            lang,
            "Kikundi kitasajiliwa rasmi baada ya ukaguzi na ofisi ya kata.",
            "Group will be officially registered after ward office review.",
          )}
        </p>
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 max-w-xs mx-auto">
          ...
          <p className="text-xs text-emerald-600">
            {vals.members.length + vals.leaders.length}{" "}
            {L(lang, "wanachama walisajiliwa", "members registered")}
          </p>
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1 justify-center flex-wrap">
        {STEPS.map((s, i) => (
          <div
            key={s.key}
            className={`px-2 py-1 rounded-full text-[9px] font-bold flex items-center gap-0.5 ${i <= stepIdx ? "bg-emerald-100 text-emerald-700" : "bg-stone-100 text-stone-400"}`}
          >
            {i < stepIdx && <Check size={9} />}
            {lang === "sw" ? s.sw : s.en}
          </div>
        ))}
      </div>
        ...
        <div className="space-y-4">
          <div className={secHdr}>
            <p className="font-bold text-emerald-800 text-sm flex items-center gap-2">
              <Users size={15} />
              {L(lang, "TAARIFA ZA KIKUNDI", "GROUP INFORMATION")}
            </p>
          </div>
          <div>
            <label className={lbl}>
              {L(lang, "Jina la Kikundi / Chama *", "Group / Association Name *")}
            </label>
            <input
              value={vals.group_name}
              onChange={(e) => {
                set("group_name", e.target.value);
                clrErr("group_name");
              }}
              className={inputCls("group_name")}
              placeholder={L(lang, "Mfano: Kikundi cha Akiba Jijini", "E.g. City Savings Group")}
            />
            <ErrMsg k="group_name" />
          </div>
          <div>
            ...
            <select
              value={vals.group_type}
              onChange={(e) => {
                set("group_type", e.target.value);
                clrErr("group_type");
              }}
              className={inputCls("group_type")}
            >
          ...
          {vals.group_type === "NYINGINE" && (
            <div>
              ...
              <input
                value={vals.group_type_other}
                onChange={(e) => set("group_type_other", e.target.value)}
                className={inputCls()}
              />
            </div>
          )}
          <div>
            ...
            <textarea
              value={vals.group_purpose}
              onChange={(e) => {
                set("group_purpose", e.target.value);
                clrErr("group_purpose");
              }}
              rows={3}
              className={`${inputCls("group_purpose")} resize-none`}
              placeholder={L(
                lang,
                "Eleza madhumuni makuu ya kikundi...",
                "Describe the main objectives of the group...",
              )}
            />
      ...
      {step === "mahali" && (
        <div className="space-y-4">
          <div className={secHdr}>
            <p className="font-bold text-emerald-800 text-sm flex items-center gap-2">
              <MapPin size={15} />
              {L(lang, "MAHALI PA MIKUTANO", "MEETING LOCATION")}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <input
              ...
              className={inputCls()}
              placeholder={L(lang, "Jina la mtaa au kijiji", "Street or village name")}
            />
          </div>
          <div>
            <label className={lbl}>
              {L(lang, "Mahali Halisi pa Mikutano", "Exact Meeting Venue")}
            </label>
            <input
              value={vals.meeting_location_detail}
              onChange={(e) => set("meeting_location_detail", e.target.value)}
              className={inputCls()}
              placeholder={L(
                lang,
                "Mfano: Kanisa la Pasua, Jengo la Jumuiya, Ukumbi wa Kata...",
                "E.g. Pasua Church Hall, Community Center, Ward Hall...",
              )}
            />
          </div>
        </div>
      ...
      {step === "viongozi" && (
        <div className="space-y-5">
          <div className={secHdr}>
            <p className="font-bold text-emerald-800 text-sm flex items-center gap-2">
              <User size={15} />
              {L(lang, "VIONGOZI VYA KIKUNDI", "GROUP LEADERS")}
            </p>
            <p className="text-xs text-emerald-600 mt-1">
              {L(
                lang,
                "Viongozi wote lazima wawe wamesajiliwa kwenye E-Mtaa na wana CT ID",
                "All leaders must be registered on E-Mtaa and have a CT ID",
              )}
            </p>
          </div>
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex gap-2">
            ...
            <p className="text-xs text-amber-700">
              {L(
                lang,
                "CT ID inapatikana kutoka kwa ofisi ya mtaa au akaunti ya E-Mtaa ya kiongozi husika.",
                "CT ID is obtained from the ward office or the leader's E-Mtaa account.",
              )}
            </p>
          </div>
          {vals.leaders.map((leader, i) => (
            <div key={i} className="border border-stone-200 rounded-2xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setLeader(i, "first_name", e.target.value);
                      clrErr(`lfn_${i}`);
                    }}
                    className={inputCls(`lfn_${i}`)}
                    placeholder={L(lang, "Jina la kwanza", "First name")}
                  />
                  <ErrMsg k={`lfn_${i}`} />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setLeader(i, "last_name", e.target.value);
                      clrErr(`lln_${i}`);
                    }}
                    className={inputCls(`lln_${i}`)}
                    placeholder={L(lang, "Jina la mwisho", "Last name")}
                  />
                  <ErrMsg k={`lln_${i}`} />
                </div>
              </div>
              <div>
                <label className={lbl}>
                  {L(lang, "CT ID (Utambulisho wa Mkazi) *", "CT ID (Citizen ID) *")}
                </label>
                <input
                  value={leader.ct_id}
                  onChange={(e) => {
                    setLeader(i, "ct_id", e.target.value.toUpperCase());
                    clrErr(`lct_${i}`);
                  }}
                  className={inputCls(`lct_${i}`)}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    value={leader.nida}
                    onChange={(e) => {
                      setLeader(i, "nida", e.target.value);
                      clrErr(`lnida_${i}`);
                    }}
                    className={inputCls(`lnida_${i}`)}
                ...
                <div>
                  ...
                  <input
                    value={leader.phone}
                    onChange={(e) => {
                      setLeader(i, "phone", e.target.value);
                      clrErr(`lphone_${i}`);
                    }}
                    className={inputCls(`lphone_${i}`)}
      ...
      {step === "wanachama" && (
        <div className="space-y-4">
          <div className={secHdr}>
            <p className="font-bold text-emerald-800 text-sm flex items-center gap-2">
              <Users size={15} />
              {L(lang, "WANACHAMA WA KIKUNDI", "GROUP MEMBERS")}
            </p>
            <p className="text-xs text-emerald-600 mt-1">
              {L(
                lang,
                "Wanachama wote lazima wawe wamesajiliwa kwenye E-Mtaa",
                "All members must be registered on E-Mtaa",
              )}
            </p>
          </div>
          <div className="bg-stone-50 border border-stone-200 rounded-xl p-3 flex justify-between items-center">
            <span className="text-xs font-bold text-stone-600">
              {L(lang, "Wanachama waliowekwa", "Members entered")}: {vals.members.length}
            </span>
            <span className="text-xs text-stone-500">
              {L(lang, "Jumla iliyotajwa", "Declared total")}: {vals.members_count}
            </span>
          </div>
          <div className="space-y-4">
              ...
              <div key={i} className="border border-stone-200 rounded-2xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-black text-stone-600">
                    {L(lang, `Mwanachama #${i + 1}`, `Member #${i + 1}`)}
                  </p>
                ...
                <div>
                  ...
                  <input
                    value={m.ct_id}
                    onChange={(e) => {
                      setMember(i, "ct_id", e.target.value.toUpperCase());
                      clrErr(`mct_${i}`);
                    }}
                    className={inputCls(`mct_${i}`)}
                    placeholder="CT-XXXXXX"
                  />
                  <ErrMsg k={`mct_${i}`} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      value={m.nida}
                      onChange={(e) => setMember(i, "nida", e.target.value)}
                      className={inputCls()}
                      placeholder="20XXXXXXXXXXXXXXXXX"
                    />
                  </div>
          ...
          <button
            type="button"
            onClick={addMember}
            className="w-full py-3 border-2 border-dashed border-emerald-300 rounded-xl text-sm font-bold text-emerald-600 hover:bg-emerald-50 flex items-center justify-center gap-2 transition-colors"
          >
            <Plus size={16} />
            {L(lang, "Ongeza Mwanachama", "Add Member")}
          </button>
        </div>
      )}
      ...
      {step === "benki" && (
        <div className="space-y-4">
          <div className={secHdr}>
            <p className="font-bold text-emerald-800 text-sm flex items-center gap-2">
              <Building2 size={15} />
              {L(lang, "TAARIFA ZA BENKI YA KIKUNDI", "GROUP BANK ACCOUNT DETAILS")}
            </p>
            <p className="text-xs text-emerald-600 mt-1">
              {L(
                lang,
                "Taarifa hizi ni hiari lakini zinahitajika kwa vikoba na SACCOS",
                "Optional but required for savings groups / SACCOS",
              )}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <input
              value={vals.bank_branch}
              onChange={(e) => set("bank_branch", e.target.value)}
              className={inputCls()}
              placeholder={L(lang, "Mfano: Tawi la Kariakoo", "E.g. Kariakoo Branch")}
            />
          </div>
          <div>
          ...
          <div>
            <label className={lbl}>
              {L(lang, "Jina la Akaunti (Account Name)", "Account Name")}
            </label>
            <input
              value={vals.account_name}
          ...
          <SignaturePad
            ...
            lang={lang}
            label={L(lang, "Saini ya Mwenyekiti", "Chairperson Signature")}
          />
        </div>
      )}
      ...
      {step === "preview" && (
        <div className="space-y-4">
          ...
          <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest">
            {L(lang, "Taarifa za Kikundi", "Group Info")}
          </p>
          <div className="bg-white border border-stone-200 rounded-xl divide-y divide-stone-100">
            {[
              ["Jina / Name", vals.group_name],
              ["Aina / Type", GROUP_TYPES.find((t) => t.value === vals.group_type)?.label || ""],
              ["Wanachama / Members", vals.members_count],
              ["Mahali / Location", `${vals.ward}, ${vals.district}, ${vals.region}`],
              ["Mkutano / Venue", vals.meeting_location_detail || "—"],
            ].map(([k, v]) => (
            ...
            {vals.leaders.map((l, i) => (
              <div key={i} className="bg-stone-50 rounded-xl px-4 py-3">
                ...
                <p className="text-xs text-stone-500">
                  CT ID: {l.ct_id} · NIDA: {l.nida} · {l.phone}
                </p>
              </div>
            ))}
          </div>
          ...
          <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest">
            {L(lang, "Wanachama", "Members")} ({vals.members.length})
          </p>
          <div className="space-y-1.5 max-h-40 overflow-y-auto">
            {vals.members.map((m, i) => (
              <div
                key={i}
                className="bg-stone-50 rounded-xl px-4 py-2.5 flex items-center justify-between"
              >
                <div>
                  <p className="text-sm font-bold">
          ...
          {vals.bank_name && (
            <>
              ...
              <div className="bg-white border border-stone-200 rounded-xl divide-y divide-stone-100">
                {[
                  ["Benki", vals.bank_name],
                  ["Tawi / Branch", vals.bank_branch || "—"],
                  ["Namba / Account No.", vals.account_number],
                  ["Jina / Account Name", vals.account_name],
                ].map(([k, v]) => (
                  <div key={k} className="flex justify-between px-4 py-2.5 text-sm">
                    <span className="text-stone-500">{k}</span>
