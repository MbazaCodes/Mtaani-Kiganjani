...
export const OmbiMsaadaJamiiForm: React.FC<FormProps> = ({
  onSubmit,
  isLoading,
  lang = "sw",
  userProfile,
  ...
  const validate = (): boolean => {
    ...
    if (step === "hali") {
      if (!vals.assistance_type)
        e.assistance_type = L(lang, "Aina ya msaada inahitajika", "Assistance type required");
      if (!vals.family_status)
        e.family_status = L(lang, "Hali ya familia inahitajika", "Family status required");
      if (!vals.problem_description.trim())
        e.problem_description = L(
          lang,
          "Maelezo ya tatizo yanahitajika",
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      await onSubmit({ ...vals, service_fee: 0, total_fee: 0 });
      setSubmitted(true);
    } finally {
      setSubmitting(false);
    }
    ...
    return (
      <div className="text-center py-10 space-y-4">
        ...
        <p className="text-stone-500 text-sm">
          {L(
            lang,
            "Ombi lako la msaada litashughulikiwa na Afisa Ustawi wa Jamii wa kata yako.",
            "Your welfare request will be handled by your ward Social Welfare Officer.",
          )}
        </p>
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 max-w-xs mx-auto text-xs text-blue-700">
          {L(
            lang,
            "Huduma hii ni bure. Utapigiwa simu ndani ya siku 3-5 za kazi.",
            "This service is free. You will be called within 3-5 working days.",
          )}
        </div>
      </div>
    );
    ...
    <div className="space-y-5">
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-center">
        <p className="text-xs font-bold text-emerald-700">
          ✅ {L(lang, "Huduma hii ni BURE — Hakuna ada", "This service is FREE — No fee charged")}
        </p>
      </div>
      ...
      {step === "hali" && (
        <div className="space-y-4">
          <div className={secHdr}>
            <p className="font-bold text-emerald-800 text-sm flex items-center gap-2">
              <Users size={15} />
              {L(lang, "HALI YA FAMILIA NA MAISHA", "FAMILY & LIVING SITUATION")}
            </p>
          </div>
          <div>
            <label className={lbl}>
              {L(lang, "Aina ya Msaada Unaohitajika *", "Type of Assistance Needed *")}
            </label>
            <select
              value={vals.assistance_type}
              onChange={(e) => {
                set("assistance_type", e.target.value);
                clrErr("assistance_type");
              }}
              className={inputCls("assistance_type")}
            >
          ...
          {vals.assistance_type === "NYINGINE" && (
            <div>
              <label className={lbl}>
                {L(lang, "Eleza Aina ya Msaada", "Describe Assistance Type")}
              </label>
              <input
                value={vals.assistance_other}
                onChange={(e) => set("assistance_other", e.target.value)}
                className={inputCls()}
              />
            </div>
          )}
          <div>
            <label className={lbl}>{L(lang, "Hali ya Familia *", "Family Situation *")}</label>
            <select
              value={vals.family_status}
              onChange={(e) => {
                set("family_status", e.target.value);
                clrErr("family_status");
              }}
              className={inputCls("family_status")}
            >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={lbl}>{L(lang, "Wanafamilia Wote", "Total Family Members")}</label>
              <input
                type="number"
                min="1"
                value={vals.family_members}
            ...
            <div>
              <label className={lbl}>
                {L(lang, "Watoto / Wategemezi", "Children / Dependents")}
              </label>
              <input
                type="number"
                min="0"
                value={vals.dependents}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={lbl}>
                {L(lang, "Mapato ya Kila Mwezi (TSh)", "Monthly Income (TSh)")}
              </label>
              <input
                type="number"
                min="0"
                value={vals.monthly_income}
            ...
            <div>
              <label className={lbl}>{L(lang, "Chanzo cha Mapato", "Income Source")}</label>
              <input
                value={vals.income_source}
                onChange={(e) => set("income_source", e.target.value)}
                className={inputCls()}
                placeholder={L(lang, "Kilimo, biashara ndogo...", "Farming, petty trade...")}
              />
            </div>
          </div>
          <div>
            <label className={lbl}>
              {L(lang, "Maelezo ya Tatizo / Hali Ngumu *", "Description of Problem / Hardship *")}
            </label>
            <textarea
              value={vals.problem_description}
              onChange={(e) => {
                set("problem_description", e.target.value);
                clrErr("problem_description");
              }}
              rows={4}
              className={`${inputCls("problem_description")} resize-none`}
              placeholder={L(
                lang,
                "Eleza kwa undani hali yako na sababu ya kuomba msaada...",
                "Describe your situation and reason for requesting assistance in detail...",
              )}
            />
          </div>
        ...
        <div className="space-y-4">
          <div className={secHdr}>
            <p className="font-bold text-emerald-800 text-sm">
              {L(lang, "MAELEZO YA OMBI", "REQUEST DETAILS")}
            </p>
          </div>
          <div>
            <label className={lbl}>
              {L(lang, "Msaada Halisi Unaohitajika", "Specific Assistance Requested")}
            </label>
            <textarea
              value={vals.assistance_needed}
              onChange={(e) => set("assistance_needed", e.target.value)}
              rows={3}
              className={`${inputCls()} resize-none`}
              placeholder={L(
                lang,
                "Mfano: Msaada wa chakula kwa miezi 3, au chakula kwa watoto 4 wadogo...",
                "E.g. Food assistance for 3 months, or food for 4 young children...",
              )}
            />
          </div>
          <div>
            <label className={lbl}>
              {L(
                lang,
                "Je, Umewahi Kupata Msaada wa Serikali Awali?",
                "Have You Previously Received Government Assistance?",
              )}
            </label>
            <textarea
              value={vals.previous_assistance}
              onChange={(e) => set("previous_assistance", e.target.value)}
              rows={2}
              className={`${inputCls()} resize-none`}
              placeholder={L(
                lang,
                "Ndiyo / Hapana. Ikiwa ndiyo, eleza aina na mwaka...",
                "Yes / No. If yes, describe type and year...",
              )}
            />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className={lbl}>
                {L(lang, "Jina la Shahidi / Jirani", "Witness / Neighbour Name")}
              </label>
              <input
          ...
          <div className="bg-stone-50 border border-stone-200 rounded-xl p-3 text-xs text-stone-600">
            {L(
              lang,
              "Ninakiri kwamba taarifa nilizotoa ni za kweli. Ninajua kwamba kutoa taarifa za uongo ni kosa la kisheria.",
              "I certify that the information I have provided is true. I understand that providing false information is a legal offense.",
            )}
          </div>
        </div>
      )}
      ...
      {step === "preview" && (
        <div className="space-y-4">
          ...
          <div className="bg-white border border-stone-200 rounded-xl divide-y divide-stone-100">
            {[
              ["Mwombaji / Applicant", vals.applicant_name],
              ["NIDA", vals.applicant_nida],
              ["Simu / Phone", vals.applicant_phone],
              ["Kata / Ward", vals.applicant_ward],
              [
                "Aina ya Msaada / Type",
                ASSISTANCE_TYPES.find((a) => a.value === vals.assistance_type)?.label || "",
              ],
              [
                "Hali ya Familia / Status",
                FAMILY_STATUS.find((f) => f.value === vals.family_status)?.label || "",
              ],
              ["Wanafamilia / Members", vals.family_members],
              ["Mapato / Income", `TSh ${Number(vals.monthly_income || 0).toLocaleString()}/mwezi`],
              ["Ada / Fee", "BURE / FREE"],
            ].map(([k, v]) => (
