    ...
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.infoBox}>
          <View style={s.field}><Text style={s.fieldLabel}>{"Jina la Kikundi / Group Name"}</Text><Text style={s.fieldValue}>{String(fd.group_name || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"Aina / Type"}</Text><Text style={s.fieldValue}>{String(fd.group_type || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"Wanachama / Members"}</Text><Text style={s.fieldValue}>{String(fd.members_count || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"Mwenyekiti / Chairman"}</Text><Text style={s.fieldValue}>{String(fd.chairman_name || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"Simu / Phone"}</Text><Text style={s.fieldValue}>{String(fd.chairman_phone || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"Katibu / Secretary"}</Text><Text style={s.fieldValue}>{String(fd.secretary_name || "")}</Text></View>
