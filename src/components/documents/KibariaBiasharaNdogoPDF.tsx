...
export const KibariaBiasharaNdogoPDF: React.FC<Props> = ({ application, lang = "sw", qrDataUrl }) => {
    ...
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.infoBox}>
          <View style={s.field}><Text style={s.fieldLabel}>{"Jina la Biashara / Business Name"}</Text><Text style={s.fieldValue}>{String(fd.business_name || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"Aina / Type"}</Text><Text style={s.fieldValue}>{String(fd.business_type || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"Anwani / Address"}</Text><Text style={s.fieldValue}>{String(fd.location_address || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"Mmiliki / Owner"}</Text><Text style={s.fieldValue}>{String(fd.owner_name || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"NIDA"}</Text><Text style={s.fieldValue}>{String(fd.owner_nida || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"Simu / Phone"}</Text><Text style={s.fieldValue}>{String(fd.owner_phone || "")}</Text></View>
          <View style={s.field}><Text style={s.fieldLabel}>{"Wafanyakazi / Employees"}</Text><Text style={s.fieldValue}>{String(fd.employees_count || "")}</Text></View>
          ...
          <View style={s.feeBox}><Text style={s.feeText}>{sw ? "Ada: TSh 10,000" : "Fee: TSh 10,000"}</Text></View>
        </View>
