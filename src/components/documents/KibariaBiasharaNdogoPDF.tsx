/**
 * KibariaBiasharaNdogoPDF — Kibari cha Biashara Ndogo / Small Business Permit
 */
...
export const KibariaBiasharaNdogoPDF: React.FC<Props> = ({
  application,
  lang = "sw",
  qrDataUrl,
}) => {
  ...
  return (
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.docTitle}>BIASHARA NDOGO</Text>
        <View style={s.refBox}>
          <Text style={s.refText}>{application.application_number}</Text>
        </View>

        <Text style={s.sectionTitle}>{sw ? "TAARIFA ZA MWOMBAJI" : "APPLICANT INFORMATION"}</Text>
        <View style={s.infoBox}>
          <View style={s.field}>
            <Text style={s.fieldLabel}>{"Jina la Biashara / Business Name"}</Text>
            <Text style={s.fieldValue}>{String(fd.business_name || "")}</Text>
          </View>
          <View style={s.field}>
            <Text style={s.fieldLabel}>{"Aina / Type"}</Text>
            <Text style={s.fieldValue}>{String(fd.business_type || "")}</Text>
          </View>
          <View style={s.field}>
            <Text style={s.fieldLabel}>{"Anwani / Address"}</Text>
            <Text style={s.fieldValue}>{String(fd.location_address || "")}</Text>
          </View>
          <View style={s.field}>
            <Text style={s.fieldLabel}>{"Mmiliki / Owner"}</Text>
            <Text style={s.fieldValue}>{String(fd.owner_name || "")}</Text>
          </View>
          <View style={s.field}>
            <Text style={s.fieldLabel}>{"NIDA"}</Text>
            <Text style={s.fieldValue}>{String(fd.owner_nida || "")}</Text>
          </View>
          <View style={s.field}>
            <Text style={s.fieldLabel}>{"Simu / Phone"}</Text>
            <Text style={s.fieldValue}>{String(fd.owner_phone || "")}</Text>
          </View>
          <View style={s.field}>
            <Text style={s.fieldLabel}>{"Wafanyakazi / Employees"}</Text>
            <Text style={s.fieldValue}>{String(fd.employees_count || "")}</Text>
          </View>
          <View style={s.field}>
            <Text style={s.fieldLabel}>{sw ? "Tarehe ya Ombi" : "Application Date"}</Text>
            <Text style={s.fieldValue}>{formatDate(application.created_at)}</Text>
          </View>
          <View style={s.field}>
            <Text style={s.fieldLabel}>{sw ? "Hali" : "Status"}</Text>
            <Text style={s.fieldValue}>
              {isApproved ? (sw ? "IMEIDHINISHWA" : "APPROVED") : sw ? "INASUBIRI" : "PENDING"}
            </Text>
          </View>
          <View style={s.feeBox}>
            <Text style={s.feeText}>{sw ? "Ada: TSh 10,000" : "Fee: TSh 10,000"}</Text>
          </View>
        </View>
        ...
        <Text style={s.validity}>
          {sw
            ? "Hati hii ni rasmi. Thibitisha kwa QR code."
            : "This document is official. Verify via QR code."}
        </Text>
      </Page>
...
export default KibariaBiasharaNdogoPDF;
