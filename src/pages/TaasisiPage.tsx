  ...
  if (selectedTaasisi && selectedCat) {
    ...
    return (
      <div className="max-w-2xl mx-auto py-6 px-4">
        <button
          onClick={() => setSelectedTaasisi(null)}
          className="inline-flex items-center gap-2 text-sm font-bold text-stone-500 hover:text-stone-800 mb-6 transition-colors"
        >
        ...
        <div className={`rounded-3xl p-6 sm:p-8 border ${col.border} ${col.bg} space-y-5`}>
          ...
          <div className="flex items-start gap-4">
            <div
              className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-black ${col.pill} shrink-0`}
            >
              {selectedTaasisi.name.slice(0, 2)}
            </div>
            <div>
              <h1 className="text-xl font-black text-stone-900">{selectedTaasisi.name}</h1>
              {selectedTaasisi.nameFull && (
                <p className={`text-sm font-bold ${col.text} mt-0.5`}>{selectedTaasisi.nameFull}</p>
              )}
              <span
                className={`inline-block mt-2 text-[10px] font-black px-2 py-0.5 rounded-full ${col.pill}`}
              >
          ...
          <div className="space-y-2.5">
            ...
            {selectedTaasisi.phone && (
              <a
                href={`tel:${selectedTaasisi.phone}`}
                className="flex items-center gap-2.5 text-sm text-stone-600 hover:text-stone-900 transition-colors"
              >
            ...
            {selectedTaasisi.email && (
              <a
                href={`mailto:${selectedTaasisi.email}`}
                className="flex items-center gap-2.5 text-sm text-stone-600 hover:text-stone-900 transition-colors"
              >
          ...
          <a
            ...
            className={`flex items-center justify-center gap-2 w-full py-3 px-5 rounded-2xl font-bold text-sm text-white transition-colors ${
              selectedCat.color === "emerald"
                ? "bg-emerald-600 hover:bg-emerald-700"
                : selectedCat.color === "blue"
                  ? "bg-blue-600 hover:bg-blue-700"
                  : selectedCat.color === "red"
                    ? "bg-red-600 hover:bg-red-700"
                    : selectedCat.color === "purple"
      ...
      <div className="max-w-3xl mx-auto py-6 px-4 space-y-5">
        ...
        <div className={`rounded-2xl p-5 border ${c.border} ${c.bg} flex items-center gap-4`}>
          ...
          <div>
            ...
            <p className="text-sm text-stone-500">
              {selectedCat.taasisi.length} {sw ? "taasisi" : "institutions"}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {selectedCat.taasisi.map((t) => (
            ...
            >
              <div
                className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 text-sm font-black ${c.pill} group-hover:scale-105 transition-transform`}
              >
                {t.name.slice(0, 2)}
              </div>
              <div className="flex-1 min-w-0">
                ...
                {t.nameFull && (
                  <p className={`text-xs font-medium ${c.text} truncate`}>{t.nameFull}</p>
                )}
    ...
    <div className="max-w-4xl mx-auto py-6 px-4 space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-stone-500 text-sm max-w-md mx-auto">
          {sw
            ? "Chagua mkundo ili uone taasisi zote zilizo ndani yake na mawasiliano yao rasmi."
            : "Select a category to see all institutions and their official contacts."}
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder={
            sw
              ? "Tafuta taasisi... (mfano: TRA, NIDA, Mahakama)"
              : "Search institutions... (e.g. TRA, NIDA, Police)"
          }
        ...
        <div className="space-y-2">
          ...
          {searchResults.length === 0 ? (
            <div className="text-center py-10 text-stone-400 text-sm">
              {sw
                ? "Hakuna taasisi inayolingana na utafutaji wako."
            ...
            searchResults.map((t) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black shrink-0 ${col.pill}`}
                  >
                    {t.name.slice(0, 2)}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <ChevronRight
                    size={15}
                    className="text-stone-300 group-hover:text-stone-500 shrink-0"
                  />
                </button>
              );
            })
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {CATEGORIES.map((cat) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-12 h-12 rounded-xl flex items-center justify-center ${col.pill} group-hover:scale-110 transition-transform`}
                >
                  {ICON_MAP[cat.icon]}
                </div>
                <div>
                  <p className="font-black text-stone-900 text-sm leading-tight">
                    {sw ? cat.nameSw : cat.nameEn}
                  </p>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex -space-x-1">
                    {cat.taasisi.slice(0, 3).map((t) => (
                      <span
                        key={t.id}
                        className={`w-6 h-6 rounded-full border-2 border-white flex items-center justify-center text-[9px] font-black ${col.pill}`}
                      >
                        {t.name.slice(0, 1)}
                      </span>
                    ))}
                  ...
                  <ChevronRight
                    size={15}
                    className={`${col.text} group-hover:translate-x-1 transition-transform`}
                  />
                </div>
              </button>
            );
