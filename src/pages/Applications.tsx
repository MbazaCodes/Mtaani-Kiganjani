...
import {
  ...
  CheckCircle2, AlertCircle, Share2, Download, Lock,
  ChevronRight, MessageSquare, Image, ExternalLink,
} from "lucide-react";
...
const stripe: Record<string, string> = {
  submitted: "bg-blue-500", pending_review: "bg-violet-500",
  pending_payment: "bg-amber-400", approved: "bg-amber-400",
  paid: "bg-teal-500", verified: "bg-teal-500",
  issued: "bg-emerald-500", rejected: "bg-red-500",
  refunded: "bg-stone-400", returned: "bg-orange-400",
};
...
export function Applications({ applications, drafts = [], onPay, onRefresh, onResumeDraft }: ApplicationsProps) {
  ...
  const hasPaid = useCallback((app: Application) =>
    !!(app.paid_at || (app.form_data as Record<string, unknown>)?.payment_data ||
      (app.payment_data as Record<string, unknown>)?.transaction_id || getFee(app) === 0),
    [getFee]);
  ...
  useEffect(() => {
    applications.filter((a) => a.status === "approved").forEach(async (app) => {
      try { await supabase.from("applications").update({ status: "pending_payment" }).eq("id", app.id).eq("status", "approved"); onRefresh?.(); } catch {}
    });
  ...
  const acceptAgreement = async (app: Application) => {
    ...
    try {
      ...
      if (!Object.keys(patch).length) { showToast(L("Si mwanachama wa mkataba huu", "Not party to this agreement"), "error"); return; }
    ...
    <div className="space-y-5">
      ...
      <AnimatePresence>
        {lightboxImg && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <img src={lightboxImg} className="max-w-full max-h-[90vh] rounded-xl object-contain" alt="Document" />
          </motion.div>
        )}
      </AnimatePresence>
          ...
          {drafts.length > 0 && (
            <div className="flex bg-stone-100 rounded-xl p-1 gap-1">
              <button onClick={() => setShowDrafts(false)}
                className={cn("px-3 h-8 rounded-lg text-xs font-bold transition-all", !showDrafts ? "bg-white text-stone-900 shadow-sm" : "text-stone-500")}>
                {L("Maombi", "Applications")}
              </button>
              <button onClick={() => setShowDrafts(true)}
                className={cn("px-3 h-8 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5", showDrafts ? "bg-white text-amber-700 shadow-sm" : "text-stone-500")}>
      ...
      {showDrafts && (
        <div className="space-y-3">
          {drafts.map((d) => (
            <div key={d.id} className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3">
              <div className="w-10 h-10 bg-amber-100 rounded-xl flex items-center justify-center shrink-0">
                <FileText size={16} className="text-amber-600" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-stone-800 text-sm truncate">{d.service_name}</p>
                <p className="text-xs text-stone-500">{new Date(d.updated_at || d.created_at).toLocaleDateString()}</p>
              </div>
        ...
        <div className="space-y-3">
          {list.length === 0 && (
            <div className="bg-white border border-stone-200 rounded-2xl p-12 text-center">
              ...
              <p className="font-bold text-stone-500">{L("Hakuna maombi", "No applications found")}</p>
            </div>
          )}

          {list.map((app, i) => {
            const name = sw ? (app.service_name || app.services?.name || "—") : (app.services?.name_en || app.service_name || app.services?.name || "—");
            ...
            return (
              <motion.div key={app.id}
                ...
                className={cn("bg-white rounded-2xl border overflow-hidden shadow-sm transition-all",
                  isOpen ? "border-emerald-400 ring-2 ring-emerald-100" : "border-stone-200 hover:border-stone-300"
                )}>
                ...
                <button className="w-full text-left p-4" onClick={() => setSelected(isOpen ? null : app)}>
                  <div className="flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <p className="font-black text-stone-900 text-sm leading-tight truncate">{name}</p>
                        <StatusBadge status={app.status} lang={lang} />
                      </div>
                      <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                        <span className="text-[11px] text-stone-400 font-mono">{app.application_number}</span>
                    ...
                    <ChevronRight size={16} className={cn("text-stone-400 shrink-0 transition-transform duration-200", isOpen && "rotate-90")} />
                  </div>
                </button>
                  ...
                  {isOpen && (
                    <motion.div
                      ...
                      <div className="p-4 space-y-4">
                        ...
                        {/* Staff feedback */}
                        {app.feedback && (
                          <div className={cn("rounded-xl p-3 border text-sm",
                            app.status === "rejected" ? "bg-red-50 border-red-200 text-red-800" : "bg-amber-50 border-amber-200 text-amber-800")}>
                            <p className="text-[10px] font-black uppercase tracking-widest mb-1 text-stone-500">{L("Maoni ya Afisa", "Officer Feedback")}</p>
                            {app.feedback}
                          </div>
                        )}

                        {/* ── PAYMENT GATE ── */}
                        {fee > 0 && !paid && app.status !== "rejected" && app.status !== "refunded" && (
                          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-4">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-amber-700 mt-0.5">{L("Lipa ili kufungua hati rasmi yako", "Pay to unlock your official document")}</p>
                              <p className="text-lg font-black text-amber-800 mt-1">{formatCurrency(fee, currency)}</p>
                            </div>
                        ...
                        {photos.length > 0 && (
                          <div>
                            ...
                            <div className="flex gap-2 flex-wrap">
                              {photos.map((p, idx) => (
                                <button key={idx} onClick={() => setLightboxImg(p.dataUrl)}
                                  ...
                                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                                    <Search size={14} className="text-white opacity-0 group-hover:opacity-100 transition-all" />
                                  </div>
                                  <p className="absolute bottom-0 left-0 right-0 text-[8px] text-white bg-black/50 px-1 py-0.5 text-center truncate">{p.type}</p>
                                </button>
                              ))}
                            </div>
                          </div>
                          ...
                          <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-center justify-between gap-3">
                            <p className="text-xs text-blue-700 font-medium flex-1">
                              {app.agreement_status === "buyer_accepted"
                                ...
                                : L("Mkataba unasubiri kukubaliwa", "Agreement awaiting acceptance")}
                            </p>
                            {app.agreement_status === "pending" && (
                              <button onClick={() => acceptAgreement(app)} disabled={processingId === app.id}
