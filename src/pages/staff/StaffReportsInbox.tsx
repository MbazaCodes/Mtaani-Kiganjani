...
import {
  Inbox, Clock, CheckCircle2, MessageSquare, ChevronRight, ChevronDown,
  Loader2, Send, MapPin, User, X, AlertTriangle, Image,
} from "lucide-react";
...
const STATUS_COLORS: Record<string, string> = {
  ...
  escalated: "bg-orange-100 text-orange-800", resolved: "bg-emerald-100 text-emerald-800",
...
interface Report {
  ...
  assigned_department_id?: string; resolution_note?: string;
  created_at: string;
  citizen?: { first_name: string; last_name: string; email: string; phone?: string };
}

interface Response {
  id: string; user_id: string; message: string; is_internal_note: boolean;
  created_at: string;
  users?: { first_name: string; last_name: string; role: string };
}

interface Media {
  id: string; media_data: string; file_name: string; media_type: string;
}
...
export function StaffReportsInbox() {
  ...
  const fetchDetail = async (reportId: string) => {
    const [respData, mediaData] = await Promise.all([
      supabase.from("report_responses").select("*, users:user_id(first_name, last_name, role)").eq("report_id", reportId).order("created_at"),
      supabase.from("community_report_media").select("id, media_data, file_name, media_type").eq("report_id", reportId),
    ]);
  ...
  const toggleExpand = (report: Report) => {
    ...
    setReplyText(""); setIsInternal(false); setNewStatus(report.status); setResolutionNote("");
  };
  ...
  const handleSendResponse = async (reportId: string) => {
    ...
    try {
      await supabase.from("report_responses").insert({
        report_id: reportId, user_id: user.id, message: replyText.trim(), is_internal_note: isInternal,
      });
      setReplyText("");
  ...
  const stats = {
    ...
    active: reports.filter((r) => ["under_review", "assigned", "in_progress"].includes(r.status)).length,
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm font-bold text-stone-500">{filtered.length} {L("taarifa", "reports")}</span>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-2 border border-stone-200 rounded-xl text-sm bg-white">
          <option value="all">{L("Zote", "All")} ({stats.total})</option>
          {STATUS_OPTIONS.map((s) => (
            <option key={s.value} value={s.value}>{sw ? s.sw : s.en} ({reports.filter((r) => r.status === s.value).length})</option>
          ))}
        </select>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map((report) => {
            ...
            return (
              <div key={report.id} className="bg-white border border-stone-200 rounded-2xl overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-stone-50/50" onClick={() => toggleExpand(report)}>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-mono text-stone-400">{report.report_number}</span>
                      <span className={cn("px-2 py-0.5 text-[10px] font-bold rounded-md uppercase", STATUS_COLORS[report.status])}>{report.status.replace(/_/g, " ")}</span>
                    </div>
                    <p className="font-bold text-stone-900 text-sm truncate">
                      {cat?.icon} {report.title}
                    </p>
                    <p className="text-xs text-stone-400">
                      {citizen?.first_name} {citizen?.last_name} · {report.ward || report.district} · {new Date(report.created_at).toLocaleDateString("sw-TZ")}
                    </p>
                  </div>
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-stone-100">
                      <div className="p-5 space-y-4">
                        <div className="bg-stone-50 rounded-xl p-4">
                          <p className="text-sm text-stone-700 whitespace-pre-wrap">{report.description}</p>
                          <div className="flex flex-wrap gap-3 mt-2 text-xs text-stone-400">
                            <span className="flex items-center gap-1"><MapPin size={10} />{report.region} / {report.district} / {report.ward}</span>
                            {report.street && <span>📍 {report.street}</span>}
                            {report.gps_lat && (
                              <a href={`https://maps.google.com/?q=${report.gps_lat},${report.gps_lng}`} target="_blank" rel="noreferrer" className="text-blue-600 underline">
                        ...
                        {media.length > 0 && (
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {media.map((m) => (
                                <img key={m.id} src={m.media_data} alt={m.file_name} className="w-24 h-24 object-cover rounded-xl border border-stone-200" />
                              ))}
                            </div>
                          </div>
                        )}
                        ...
                        {responses.length > 0 && (
                          <div className="space-y-2">
                            <p className="text-xs font-bold text-stone-500 uppercase tracking-wider">{L("Mazungumzo", "Conversation")} ({responses.length})</p>
                            {responses.map((r) => (
                              <div key={r.id} className={cn("p-3 rounded-xl text-sm", r.is_internal_note ? "bg-amber-50 border-dashed border border-amber-100" : r.users?.role !== "citizen" ? "bg-blue-50 border border-blue-100" : "bg-stone-50")}>
                                <div className="flex items-center gap-2 mb-1">
                                  <span className="text-xs font-bold text-stone-600">{r.users?.first_name} {r.users?.last_name}</span>
                                  {r.is_internal_note && <span className="text-[10px] text-amber-600 font-bold">[INTERNAL]</span>}
                        ...
                        <div className="space-y-2 pt-3 border-t border-stone-100">
                          <div className="flex gap-2">
                            <input value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder={L("Andika jibu...", "Type response...")} className="flex-1 px-4 py-2.5 border border-stone-200 rounded-xl text-sm" />
                            <button onClick={() => handleSendResponse(report.id)} disabled={sending || !replyText.trim()} className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold disabled:opacity-50 flex items-center gap-1.5">
                              {sending ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />}
                            </button>
                          </div>
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={isInternal} onChange={(e) => setIsInternal(e.target.checked)} className="w-3.5 h-3.5 rounded border-stone-300 text-amber-600" />
                            <span className="text-xs text-stone-500">{L("Maoni ya ndani", "Internal note")}</span>
                          </label>
                        </div>
                        ...
                        <div className="flex flex-wrap items-end gap-3 pt-3 border-t border-stone-100">
                          <div className="flex-1 min-w-[160px]">
                            <label className="text-xs font-bold text-stone-500 uppercase mb-1 block">{L("Badilisha Hali", "Change Status")}</label>
                            <select value={newStatus} onChange={(e) => setNewStatus(e.target.value)} className="w-full px-3 py-2 border border-stone-200 rounded-xl text-sm bg-white">
                              {STATUS_OPTIONS.map((s) => (<option key={s.value} value={s.value}>{sw ? s.sw : s.en}</option>))}
                            </select>
                          </div>
                          {newStatus === "resolved" && (
                            <div className="flex-1 min-w-[200px]">
                              <label className="text-xs font-bold text-stone-500 uppercase mb-1 block">{L("Maelezo", "Resolution Note")}</label>
                              <input value={resolutionNote} onChange={(e) => setResolutionNote(e.target.value)} placeholder={L("Jinsi ilivyotatuliwa...", "How resolved...")} className="w-full px-3 py-2 border border-stone-200 rounded-xl text-sm" />
                            </div>
                          )}
                          <button onClick={() => handleUpdateStatus(report)} disabled={updating || newStatus === report.status} className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-bold disabled:opacity-50 flex items-center gap-1.5">
